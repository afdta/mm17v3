...
var dir = {
	rackspace: 'https://c24215cec6c97b637db6-9c0895f07c3474f6636f95b6bf3db172.ssl.cf1.rackcdn.com/interactives/2016/',
	folders: {}
};

dir.local = function(root_directory){
	if(arguments.length > 0){
		dir.rackspace = root_directory;
	}
	else{
		dir.rackspace = "./";
	}
...
dir.add = function(name, relative_path){
	if(arguments.length==1){
		dir.folders[name] = name;
	}
	else if(arguments.length > 1){
		dir.folders[name] = relative_path;
	}
};

dir.url = function(name, file_name){
	if(arguments.length==0){
		return dir.rackspace;
	}
	else if(arguments.length==1){
		return dir.rackspace + dir.folders[name];
	}
	else if(arguments.length > 1){
		return dir.rackspace + dir.folders[name] + "/" + file_name;
	}
};
...
function metro_map(container, url, state, indicators){
	...
	var buttons_enter = buttons_update.enter().append("div").classed("mi-button disable-hightlight",true);
...
//v1.0 developed for congressional district poverty, gig economy, and gci types
...
ScrollCollection.prototype.register = function(element, onview, onscroll){
	if(!!element){
		...
		var event = {};
			event.viewed = false; //has the container been brought into view (and activated)
			event.setup = false; //has the view been initialized and set up? (useful because activation may be asynchronous)
			event.viewing = false;
...
ScrollCollection.prototype.activate = function(id){
	if(this.listeners.hasOwnProperty(id)){
		...
		if(box==null || window_height==0){
			...
			if(listener.setup && listener.viewing && listener.on_scroll){
...
function ranks_and_distributions(container, url, state){
  ...
  chart.width = 320;
  chart.height = 550;
  chart.wrap_outer = d3.select(container).classed("c-fix makesans",true);
  chart.wrap = chart.wrap_outer.append("div").style("width","100%");
  ...
  function build(){
    ...
    var indicators = this.get_data().measures.map(function(d){return d.key});

    var data = indicators.map(function(d,i,a){
      var D = self.get_data().values[period][d];
      return metro_data.zip(D, "cbsa", "geo"); //bind to official names
    });
    ...
    var cat = data[0].concat(data[1], data[2]);

    //var zrange = [-2,4];
    var zrange = d3.extent(cat, function(d){return d.obs.z});

    var x = d3.scaleBand().domain(indicators).range([0,100])
    ...
    //build groups
    var groups_update = chart.main_group.selectAll("g").data(data);
    ...
    groups.select("rect")
          .attr("x",function(d,i){
            var indicator = d[0].obs.measure; //to do: add in metadata to json
            return x(indicator)+"%";
          })
          .attr("y",0)
    ...
    groups.select("line")
          .attr("x1",function(d,i){return (x(d[0].obs.measure)+halfstep)+"%"})
          .attr("x2",function(d,i){return (x(d[0].obs.measure)+halfstep)+"%"})
          .attr("y1",0)
          .attr("y2",chart.height)
          .attr("stroke","#aaaaaa")
          .attr("stroke-dasharray","3,3");

    var dots = groups.selectAll("circle").data(function(d,i){
      return d.filter(function(d,i,a){return d.obs.z !== null});
    });
    dots.exit().remove();
    var alldots = dots.enter().append("circle").merge(dots)
            .attr("cx",function(d,i){return (x(d.obs.measure)+halfstep)+"%"})
            .attr("cy",function(d,i){return y(d.obs.z)})
    ...
    alldots.filter(function(d,i){
      return d.id.code == cbsa;
    }).raise().attr("r",7).attr("fill","#dc2a2a").attr("fill-opacity",1).attr("stroke-opacity",1).attr("stroke","#dc2a2a");
...
function trend_lines(container, url, state){
  ...
  //add a chart note
  chart.note = chart.wrap_outer.append("p")
...
function main(){
	...
	//headers
	sectionHeader.factory(document.getElementById("findings-section-header"), "Context", function(m){return "Situating " + m.nameshort + " in broad metropolitan trends"});
	...
	//dom setup
	var dom = {};
	dom.wrap = d3.select("#mm-wrap");


	//slope chart
	var slope = {};
	slope.wrap = d3.select("#slope-chart");
	slope.svg = slope.wrap.append("svg");
	slope.height = 750;
	slope.width = 800; //responsive, reset by build method
	slope.minwidth = 320;

	slope.wrap.style("width","100%")
			  .style("height",slope.height+"px")
			  ;

	slope.rects = null;
	slope.groups = null;

	slope.build = function(data){
		var newdata = false;

		var dim = dimensions(dom.wrap.node());


		if(dim.width < 320){var width = 320;}
		else if(dim.width > 1200){var width = 1200;}
		else{var width = dim.width;}

		var step = Math.round(width/4);

		//bind data
		if(arguments.length > 0){
			newdata = true;

			var extent = d3.extent(data.measures.growth.overall.concat(
								  data.measures.prosperity.overall,
								  data.measures.inclusion.overall), function(d,i){return d.FiveZ});

			slope.vertices = {};
			for(var i=0; i<data.measures.growth.overall.length; i++){
				var cbsa = data.measures.growth.overall[i].CBSA;
				var p1 = data.measures.growth.overall[i].FiveZ;
				var p2 = data.measures.prosperity.overall.filter(function(d,i){return d.CBSA==cbsa})[0].FiveZ;
				var p3 = data.measures.inclusion.overall.filter(function(d,i){return d.CBSA==cbsa})[0].FiveZ;

				slope.vertices[cbsa] = [p1,p2,p3];
			}

			slope.scale = d3.scaleLinear().range([slope.height-10, 10]).domain(extent);

			var p = slope.svg.selectAll("path").data(data.measures.growth.overall);
			p.exit().remove();
			slope.paths = p.enter().append("path").merge(p);

			var g = slope.svg.selectAll("g").data([data.measures.growth.overall,
												   data.measures.prosperity.overall,
												   data.measures.inclusion.overall]);
			g.exit().remove();
			slope.groups = g.enter().append("g").merge(g);

			var r = slope.groups.selectAll("rect").data(function(d,i){return d});
			r.exit().remove();
			slope.rects = r.enter().append("rect").merge(r);
			slope.rects.attr("width","10")
					   .attr("height","3")
					   .attr("x","0")
					   .attr("fill","#0d73d6")
		...
		var scale = slope.scale;
		if(slope.paths){
			slope.paths.attr("d", function(d,i){
				var v = slope.vertices[d.CBSA];
				var D = "M" + (step+10) + "," + (scale(v[0])+1) + " L"
							+ ((2*step)+0) + ","+ (scale(v[1])+1) + "m10,0" + "L"
							+ ((3*step)+0) + ","+ (scale(v[2])+1);
				return D;
			})
			.attr("fill","none")
			.attr("stroke","#e0e0e0")
		...
		slope.paths.filter(function(d,i){
			var v = slope.vertices[d.CBSA];
		...
		if(slope.groups){
			slope.groups.attr("transform",function(d,i){return "translate("+((i+1)*step)+",0)"});
		}

		if(slope.rects){
			//console.log(slope.rects);
			slope.rects.attr("y", function(d,i){
				return scale(d.FiveZ);
			});
		}

			var avg = slope.svg.selectAll("line").data([scale(0)]);
			avg.exit().remove();
			avg.enter().append("line").merge(avg)
				.attr("x1","0")
				.attr("x2","100%")
				.attr("y1",scale(0))
				.attr("y2",scale(0))
	...
	d3.json(overall, function(err, dat){
		if(!!err){
			console.log(err);
		}
		else{
			slope.build(dat);
			window.addEventListener("resize", function(){slope.build();});
		}
	});


	//var ranks_card = card(document.getElementById("rankings-chart")).build(rankings_graphic);
	//ranks_card.json(combined);
}
